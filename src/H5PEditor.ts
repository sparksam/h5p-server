...
export default class H5PEditor {
  ...
  constructor(
    ...
    this.globalCustomStyles = this.options?.customization?.global?.styles || [];
  ...
  public async getLibraryData(
    ...
    const [
      ...
      this.listAssets(
        new LibraryName(machineName, majorVersionAsNr, minorVersionAsNr),
        language
      ),
   ...
   */
  public setCopyrightSemantics(copyrightSemantics: ISemanticsEntry): H5PEditor {
   ...
   * @returns the H5PEditor object that you can use to chain method calls
   */
  public setMetadataSemantics(metadataSemantics: ISemanticsEntry[]): H5PEditor {
  ...
  public async uploadPackage(
    ...
    try {
      if (options?.onlyInstallLibraries) {
        returnValues = {
          installedLibraries:
            await this.packageImporter.installLibrariesFromPackage(filename),
        };
      } else {
        returnValues =
    ...
    Object.keys(parameters).forEach((key: string) => {
      if (key === "library" && typeof parameters[key] === "string") {
        if (parameters[key].match(/.+ \d+\.\d+/)) {
          collect[parameters[key]] = LibraryName.fromUberName(parameters[key], {
            useWhitespace: true,
          });
        }
      } else {
  ...
  private async getAddonsForLibrary(
    ...
    for (const installedAddon of installedAddons) {
      ...
      if (
        installedAddon.addTo?.editor?.machineNames?.includes(machineName) ||
    ...
    for (const addonMachineName of configRequestedAddons) {
      const installedAddonVersions =
        await this.libraryManager.listInstalledLibraries(addonMachineName);
      if (
        !neededAddons.map((a) => a.machineName).includes(addonMachineName) &&
  ...
  private getLanguageReplacer(language: string): (script: string) => string {
    ...
    if (variantList) {
      ...
      if (alternativeVariant) {
        return (f) =>
          f.replace("language/en.js", `language/${alternativeVariant}.js`);
      }
    }
    ...
    if (supportedLanguageList.includes(languageWithoutVariant)) {
      return (f) =>
        f.replace("language/en.js", `language/${languageWithoutVariant}.js`);
    }
    return (f) => f;
  }
  ...
  private listCoreStyles(): string[] {
    return editorAssetList.styles.core
      .map(this.urlGenerator.coreFile)
      .concat(
        editorAssetList.styles.editor.map(this.urlGenerator.editorLibraryFile)
      )
  ...
  private resolveDependencies(
    ...
    const resolve: (dependency: ILibraryName) => Promise<any> = (
      ...
      return this.listAssets(dependency, language, loaded)
        .then((assets: IAssets) => (assets ? resolved.push(assets) : null))
